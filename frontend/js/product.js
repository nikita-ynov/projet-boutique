...
function populatePage(p) {
  ...
  const img1 = images[0] || 'https://images.unsplash.com/photo-1559181567-c3190ca9d222?w=800&q=80';
  ...
  if (mainImg) { mainImg.src = img1; mainImg.alt = p.name; }

  // Thumbnails
  const thumbs = document.getElementById('thumbsContainer');
  if (thumbs && images.length) {
    thumbs.innerHTML = images.map((src, i) => `
      <img src="${src}" alt="${p.name} ${i + 1}" class="${i === 0 ? 'active' : ''}" data-src="${src}" loading="lazy">
    `).join('');
  ...
  if (badge) {
    if (p.discount > 20) { badge.textContent = '🔥 Promo'; badge.style.display = 'inline-flex'; }
  ...
  toggleBtn?.addEventListener('click', () => {
    expanded = !expanded;
    if (descEl) descEl.classList.toggle('expanded', expanded);
    if (toggleBtn) toggleBtn.textContent = expanded ? 'Voir moins ↑' : 'Voir plus ↓';
  });

  // Rating (fictif pour l'instant, pas dans le schéma)
  ...
  if (stockEl) {
    ...
    stockEl.style.color = p.stock === 0 ? 'var(--accent)' : '';
  }

  // Couleurs
  const colorsEl = document.getElementById('colorsEl');
  const colorsList = Array.isArray(p.colors) ? p.colors : (typeof p.colors === 'string' ? p.colors.split(',').map(c => c.trim()) : []);
  ...
  const sizesList = Array.isArray(p.sizes) ? p.sizes : (typeof p.sizes === 'string' ? p.sizes.split(',').map(s => s.trim()) : []);
  ...
  document.getElementById('qtyPlus')?.addEventListener('click', () => {
    if (p.stock === 0) return;
    if (qty < p.stock) { qty++; if (qtyEl) qtyEl.textContent = qty; }
  });
